...
Composing 4K on the VPS is NOT allowed — 4 CPU cores, no GPU, and the live console runs there.
...
1. Cut each scene from `body.mp4` at `recording.srcOffsetsMs[i]` for `durMs` — an EXACT
   ...
   No zoom — the frame is fixed (avatar-placement.md).
...
`build_cues()` — sentence-first chunking, comma splits, hard-wrap at 90 chars. Two render
