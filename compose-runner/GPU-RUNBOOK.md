...
An H100 box (the LongCat avatar model wants it). ffmpeg with `h264_nvenc`, which is standard
on any CUDA image (`nvidia/cuda` + `apt install ffmpeg`, or a prebuilt ffmpeg-nvenc image). Also:
...
If anything errors, fix it here. The box is fast and iteration is cheap.
...
boundaries. **NO music**, which is the owner's project-wide rule (review-notes.md line 9: "No music
anywhere in the project ... keep it out of the final composite too"). (Script to be written
on the box, using pure ffmpeg xfade + concat + metadata.)
...
- Frame is FIXED, with no zoom (avatar-placement.md). The runner suppresses it for `bare`.
